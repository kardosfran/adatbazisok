...
WITH p.name AS Director, count(m) AS MovieCount
WHERE MovieCount > 1
...
RETURN m.title AS Title, count(p) AS ActorCount
...
CREATE (:Tanulo {Nev: "Nagy Ilona", Eletkor: 23, Atlag: 4.4});

// Tanárok létrehozása
CREATE (:Tanar {Nev: "Tóth Ottó", Szak: "matematika"});
CREATE (:Tanar {Nev: "Nagy Ivett", Szak: "informatika"});

// Csomópontok lekérdezése
MATCH (n)
RETURN n
...
MATCH (tanar:Tanar {Nev: "Tóth Ottó"}), (tanulo:Tanulo {Nev: "Kiss Béla"})
CREATE (tanar)-[:Tanit]->(tanulo);

// Nagy Ivett tanítja Nagy Ilonát
MATCH (tanar:Tanar {Nev: "Nagy Ivett"}), (tanulo:Tanulo {Nev: "Nagy Ilona"})
CREATE (tanar)-[:Tanit]->(tanulo);
...
```js
MATCH (t:Tanulo {Nev: "Nagy Ilona"})
SET t.Atlag = 5.0

MATCH (t:Tanar {Nev: "Tóth Ottó"})
SET t.Szak = "Fizika"
